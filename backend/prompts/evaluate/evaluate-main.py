...
1. ALL sections with their complete evaluations
2. ALL sentence evaluations within each section
3. ALL requirement evaluations for each applicable requirement
4. The complete article_evaluation with ALL applicable requirements
...
IMPORTANT: Your response MUST be a complete EvaluationOutput object structured EXACTLY as shown in the following JSON format. Any deviation from this format or omission of any evaluation components will cause validation errors. You MUST include:
1. ALL sections with their complete evaluations
2. ALL sentence evaluations within each section
3. ALL requirement evaluations for each applicable requirement
4. The complete article_evaluation with ALL applicable requirements
...
if __name__ == "__main__":
    ...
    with open("evaluation-APRT-wikipedia.json", "w") as outfile:
        json.dump(evaluation_output.model_dump(), outfile, indent=4)

    # Load and process markdown file instead of JSON
    with open("APRT-wikicrow.json") as art:
        article = json.load(art)
    ...
    # Save the final JSON to a file
    with open("evaluation-APRT-wikicrow.json", "w") as outfile:
        json.dump(evaluation_output.model_dump(), outfile, indent=4)
